...
function MobileInterface({ products, inventory, rawMaterials, clientPurchaseOrders, customers, vendors, shipments, invoices, onRefresh, setActiveTab }) {
  ...
  const BottomNav = React.createElement('div', { className: 'fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50' },
  ...
  if (mobileView === 'home') {
    return React.createElement('div', { className: 'relative' },
      React.createElement('div', { className: 'min-h-screen bg-gray-100 pb-20' },
      // Header
...
function InventoryViewEx({ inventory, rawMaterials, products, customers, onRefresh, filter, setFilter, rangeMode, setRangeMode }){
  ...
  async function refreshStockAdjustments() {
    try {
      const resp = await fetch(`${API_URL}/stock-adjustments`);
      if (resp.ok) {
        const data = await resp.json();
  ...
  async function createStockAdjustment() {
    if (!stockAdjustmentForm.product_id || !stockAdjustmentForm.reason || !stockAdjustmentForm.quantity) {
      alert('Please fill in Product, Quantity, and Reason');
      return;
    }

    try {
      const res = await fetch(`${API_URL}/stock-adjustments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        const error = await res.json();
        alert(`Failed to create stock adjustment: ${error.error || 'Unknown error'}`);
      }
    } catch (err) {
      alert(`Error: ${err.message}`);
    }
  ...
  async function deleteStockAdjustment(id) {
    if (!confirm('Delete this stock adjustment? This will reverse the inventory change.')) return;

    try {
      const res = await fetch(`${API_URL}/stock-adjustments/${id}`, { method: 'DELETE' });
      if (res.ok) {
        alert('Stock adjustment deleted and inventory reversed');
        await refreshStockAdjustments();
        if (onRefresh) await onRefresh();
      } else {
